...
-- ----------------------------------
-- adição de users & processos (18-12-2023)
-- ----------------------------------
...
(
    ...
    -- ------------------
    atendimento_id                BIGINT unsigned UNIQUE                       	   DEFAULT NULL,
    -- ------------------
...
--  ------------------------------------------------------------------------
--  adições derivadas de cuidados pessoais (18-12-2023)
--  ------------------------------------------------------------------------
...
-- DROP TABLE IF EXISTS razao_toma;
CREATE TABLE IF NOT EXISTS razao_toma (
...
CREATE TABLE IF NOT EXISTS toma_medicamento  (
   id  BIGINT(19) UNSIGNED NOT NULL AUTO_INCREMENT,
   inscricao_id  BIGINT(19) UNSIGNED NOT NULL,
   medicamento_id  BIGINT(19) UNSIGNED NOT NULL,
   razao_toma_id  TINYINT(3) UNSIGNED NOT NULL,
   descricao_dosagem  VARCHAR(255) NULL DEFAULT '',
   dh_toma  DATETIME NULL DEFAULT NULL,
   ano_letivo  INT(10) UNSIGNED NOT NULL DEFAULT '2023',
   sala  TINYINT(3) UNSIGNED NULL DEFAULT '0',
   reg_por_user_id  BIGINT(19) UNSIGNED NOT NULL,
   dh_confirmacao_educadora  DATETIME NULL DEFAULT NULL,
   educadora_user_id  BIGINT(19) UNSIGNED NOT NULL,
   created_at  TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
   updated_at  DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY ( id ))ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci;
...
--  ------------------------------------------------------------------------
  ...
  -- -- DROP INDEX toma_medicamento_inscricao_id_idx 		ON toma_medicamento,
  -- -- DROP INDEX toma_medicamento_medicamento_id_idx 	ON toma_medicamento,
  -- -- DROP INDEX toma_medicamento_razao_toma_id_idx 		ON toma_medicamento,
  -- -- DROP INDEX fk_toma_medicamento_reg_por_user_id 	ON toma_medicamento,
  -- -- DROP INDEX fk_toma_medicamento_educadora_user_id 	ON toma_medicamento,
  CREATE INDEX  toma_medicamento_inscricao_id_idx  		ON toma_medicamento	( inscricao_id       ASC) ;
  CREATE INDEX  toma_medicamento_medicamento_id_idx  	ON toma_medicamento	( medicamento_id     ASC) ;
  CREATE INDEX  toma_medicamento_razao_toma_id_idx  	ON toma_medicamento	( razao_toma_id      ASC) ;
  CREATE INDEX  fk_toma_medicamento_reg_por_user_id  	ON toma_medicamento	( reg_por_user_id    ASC) ;
  CREATE INDEX  fk_toma_medicamento_educadora_user_id  	ON toma_medicamento	( educadora_user_id  ASC) ;

	ALTER TABLE toma_medicamento
		ADD CONSTRAINT fk_toma_medicamento_inscricao_id
			FOREIGN KEY (inscricao_id)
				REFERENCES inscricao (id)
				ON DELETE RESTRICT
				ON UPDATE CASCADE;
	...
	ALTER TABLE toma_medicamento
		ADD CONSTRAINT fk_toma_medicamento_razao_toma_id 
	...
	ALTER TABLE toma_medicamento
		ADD CONSTRAINT fk_toma_medicamento_medicamento_id
			FOREIGN KEY (razao_toma_id)
				REFERENCES razao_toma (id)
				ON DELETE RESTRICT
				ON UPDATE CASCADE;
                
	ALTER TABLE toma_medicamento
		ADD CONSTRAINT fk_toma_medicamento_reg_por_user_id
			FOREIGN KEY (reg_por_user_id)
				REFERENCES users (id)
				ON DELETE RESTRICT
				ON UPDATE CASCADE;
...
CREATE TABLE ocorrencia (
    ...
    inscricao_id BIGINT UNSIGNED NOT NULL,
    ano_letivo INT(10) UNSIGNED NOT NULL DEFAULT 2023,
    sala TINYINT(3) NOT NULL DEFAULT 0,
    -- estados
	-- 0 - Eliminada
    -- 1 - Em edição
    -- 2 - Registada
    ...
    reg_por_colaborador_id BIGINT(19) UNSIGNED NOT NULL,
    descricao_ferimentos 							TEXT,
    notificacao_policial 							TINYINT(2),
    notificacao_ministerio_publico 					TINYINT(2),
    exame_medico  			 						TINYINT(2),
    comunicacao_familia 			 				TINYINT(2),
    comunicacao_interna 							TEXT,
    categoria_incidente 							VARCHAR(100),
    -- -----
	data_hora_acidente								TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- -----
    local_acidente 									VARCHAR(200) DEFAULT 'JDI',
    agressao_fisica_si_proprio 						TINYINT(2),
    agressao_fisica_familia 						TINYINT(2),
    agressao_fisica_outro 							TINYINT(2),
    agressao_fisica_especifique 					TEXT,
    dano_na_crianca_acidente 						TINYINT(2),
    dano_na_crianca_si_proprio 						TINYINT(2),
    dano_na_crianca_familia 						TINYINT(2),
    dano_na_crianca_outro 							TINYINT(2),
    dano_na_crianca_especifique 					TEXT,
    ingestao_substancias_suspeita_observada 		TINYINT(2),
    ingestao_substancias_admitida 					TINYINT(2),
    ingestao_substancias_med_documentada 			TINYINT(2),
    ingestao_substancias_outros 					TINYINT(2),
    ingestao_substancias_especifique 				TEXT,
    comportamentos_negativos_ameaca 				TINYINT(2),
    comportamentos_negativos_contato_policial   	TINYINT(2),
    comportamentos_negativos_med_documentada 		TINYINT(2),
    comportamentos_negativos_outros 				TINYINT(2),
    comportamentos_negativos_especifique 			TEXT,
    abuso_sexual_comportamento_improprio_utente 	TINYINT(2),
    abuso_sexual_comportamento_improprio_colaborador TINYINT(2),
    abuso_sexual_comportamento_improprio_familia 	TINYINT(2),
    abuso_sexual_comportamento_improprio_quem 		TEXT,
    abuso_sexual_comportamento_improprio_outro 		TINYINT(2),
    abuso_sexual_comportamento_improprio_especifique TEXT,
    alegacao_abusos_colaborador 					TINYINT(2),
    alegacao_abusos_familia 						TINYINT(2),
    alegacao_abusos_quem 							TEXT,
    alegacao_abusos_outro 							TINYINT(2),
    alegacao_abusos_especifique 					TEXT,
    tipo_de_alegacao_fisico	 						TINYINT(2),
    tipo_de_alegacao_sexual 						TINYINT(2),
    tipo_de_alegacao_negligencia 					TINYINT(2),
    tipo_de_alegacao_outro 							TINYINT(2),
    tipo_de_alegacao_especifique 					TEXT,
    ficha_ocorrencia_nao_investigado 				TINYINT(2),
    ficha_ocorrencia_decisao_pendente 				TINYINT(2),
    ficha_ocorrencia_investigado 					TINYINT(2),
    acoes_negativas_verbal 							TINYINT(2),
    acoes_negativas_fisica 							TINYINT(2),
    acoes_negativas_outro 							TINYINT(2),
    acoes_negativas_especifique 					TEXT,
    familia_magoada_contencao 						TINYINT(2),
    familia_magoada_infligido 						TINYINT(2),
    familia_magoada_outro 							TINYINT(2),
    familia_magoada_especifique 					TEXT,
    fonte_observacao_colaboradores					TINYINT(2),
    fonte_observacao_familia					 	TINYINT(2),
    fonte_observacao_outro 							TINYINT(2),
    fonte_observacao_especifique 					TEXT,
    ocorrencia_privada 								TINYINT(2),
    ocorrencia_publica 								TINYINT(2),
    -- -----------------------------------
...
CREATE TABLE avaliacao_ocorrencia (
    ...
    notificacao_ministerio_publico TINYINT(2),
    exame_medico TINYINT(2),
    comunicacao_pessoas_significativas TINYINT(2),
    observacoes TEXT,
...
ALTER TABLE ocorrencia
    ADD CONSTRAINT fk_ocorrencia_inscricao_id
        FOREIGN KEY (inscricao_id)
            REFERENCES inscricao (id)
            ON DELETE RESTRICT
            ON UPDATE CASCADE;
...
ALTER TABLE ocorrencia
    ADD CONSTRAINT fk_ocorrencia_users_id
        FOREIGN KEY (reg_por_colaborador_id)
            REFERENCES users (id)
            ON DELETE RESTRICT
            ON UPDATE CASCADE;
...
CREATE TABLE projeto_curricular (
    ...
    dh_submetido 	  TIMESTAMP     	NOT NULL 		DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE relatorio_pc (
    ...
    submetido_por 				VARCHAR(255)  NOT NULL,
    dh_relatorio 				TIMESTAMP 	  NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE Plano_Semanal_de_Sala(
    ...
    -- -----------------------
    dh_avaliacao TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	-- -----------------------
	dh_eliminado TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
